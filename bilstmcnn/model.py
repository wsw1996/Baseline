...
#
#
# class TrajbiLSTM(nn.Module):
#     def __init__(self,obs_len,pred_len,input_size,hidden_size,output_size,num_layers):
#         super(TrajbiLSTM, self).__init__()
#         self.obs_len = obs_len
#         self.pred_len = pred_len
#         self.input_size = input_size
#         self.hidden_size = hidden_size
...
#         self.fc1 = nn.Linear(2*obs_len*hidden_size,pred_len*hidden_size)
#         self.fc2 = nn.Linear(hidden_size,output_size)
#
#     def forward(self,x):
#         xa,_ = self.lstm(x)          #obs,batch,2*hidden
#         seq,b,h = xa.shape
#         xa = xa.shape(b,-1)
#         xa = self.fc1(xa)
#         xa = xa.shape(b,self.pred_len,-1)
...
# class TrajLSTMcnn(nn.Module):
#     def __init__(self,obs_len,pred_len,input_size,hidden_size,output_size,num_layers,kernel_size):
#         super(TrajLSTMcnn, self).__init__()
#         self.obs_len = obs_len
#         self.pred_len = pred_len
#         self.input_size = input_size
#         self.hidden_size = hidden_size
...
#         self.conv1 = nn.Conv1d(obs_len,pred_len,kernel_size,padding=1)
#
#     def forward(self,x):
#         xa,_ = self.lstm(x)          #obs,batch,hidden
#         seq,b,h = xa.shape
#         xa = xa.permute(1,0,2)
#         xa = self.conv1(xa)    #(batch,pred_len,hidden)
#         xa = self.fc1(xa)
#         xa = xa.view(self.pred_len,b,-1)
#
#         return xa
...
class TrajBiLSTMcnn(nn.Module):
    def __init__(self,obs_len,pred_len,input_size,hidden_size,output_size,num_layers,kernel_size):
        super(TrajBiLSTMcnn, self).__init__()
        self.obs_len = obs_len
        self.pred_len = pred_len
        self.input_size = input_size
        self.hidden_size = hidden_size
        ...
        self.conv1 = nn.Conv1d(obs_len,pred_len,kernel_size=kernel_size,padding=1)
        self.relu = nn.ReLU()

    def forward(self,x):
        x = x.permute(1,0,2)
        x = self.conv1(x)    #(batch,pred_len,input_size)
        x = self.relu(x)
        x = x.permute(1,0,2)    #(seq,batch,input-size)
        x,_ = self.lstm(x)
        s, b, h = x. shape
        x = x.view(s * b, h)
        x = self.fc1(x)
...
#         # temporal_zero = torch.zeros_like(temporal_interaction_mask, device='cuda')   #这块改一下
#
#         spatial_interaction_mask = torch.where(spatial_interaction_mask > threshold, spatial_interaction_mask,
#                                                0)
# #这块和论文描述不对应，大于阈值就是激活之后的interaction_mask本身，小于阈值是0
#         temporal_interaction_mask = torch.where(temporal_interaction_mask > threshold, temporal_interaction_mask,
#                                                0)
#         # spatial_interaction_mask = torch.zeros_like(spatial_interaction_mask, device='cuda')
#         # temporal_interaction_mask = torch.zeros_like(temporal_interaction_mask, device='cuda')   #这块改一下
#
...
#
#         gcn_spatial_features = self.spatial_temporal_sparse_gcn[0](spa_graph, normalized_spatial_adjacency_matrix)  # [batch_size num_heads seq_len hidden_size]
...
#
#         self.encoder = Encoder(fin=self.obs_len,fout=self.pred_len)
#
#
#     def forward(self, graph, identity):
